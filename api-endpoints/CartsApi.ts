...
export const updateSelectedAddressApi = async (path: any, payload: any) => {
    const formattedPath = path.endsWith('/') ? path : `${path}/`;
    return axios.patch(`${ApiUrls.updateSelectedAddress}/${formattedPath}`, payload);
};

export const getDeliveryChargeApi = async (query: any, payload: any) => {
    return axios.post(`${ApiUrls.checkCourierCharge}${query}`, payload);
};
