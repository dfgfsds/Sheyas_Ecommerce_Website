...
export default function ProductDetailPage() {
    ...
    React.useEffect(() => {
        if (productData) {
            ...
            if (productData.variants?.length > 0 && productData.variants[0].sizes?.length > 0) {
                // Find first available size to auto select, instead of just the first one
                const firstAvailableSize = productData.variants[0].sizes.find((s: any) => s.product_size_status !== false);
                setSelectedSize(firstAvailableSize || productData.variants[0].sizes[0]);
            }
        }
    }, [productData]);
                ...
                <div className="flex flex-col pt-4">
                    ...
                    {productData?.variants?.length > 0 && (
                        <div className="mb-10">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                {productData?.variants?.map((variant: any) => {
                                    ...
                                    // 🛠️ Variant Status Check Logic
                                    const isVariantAvailable = variant?.product_variant_status !== false;

                                    return (
                                        <button
                                            key={variant?.id}
                                            disabled={!isVariantAvailable} // Status false ah iruntha button disable aagidum
                                            onClick={() => {
                                                setSelectedVariant(variant);
                                                setPreviewImage(
                                                    variant?.product_variant_image_urls?.[0] || ""
                                                ...
                                                // RESET SIZE
                                                if (variant?.sizes?.length > 0) {
                                                    const firstAvail = variant?.sizes.find((s: any) => s.product_size_status !== false);
                                                    setSelectedSize(firstAvail || variant?.sizes[0]);
                                                } else {
                                                    setSelectedSize(null);
                                                }
                                            }}
                            ...
                            border transition-all duration-300
                            ${!isVariantAvailable
                                                    ? "bg-gray-50 border-gray-200 opacity-50 cursor-not-allowed" // Disabled Styling
                                                    : isSelected
                                                        ? "border-black shadow-2xl scale-[1.02]"
                                                        : "border-gray-200 hover:border-black hover:shadow-lg"
                                                }
                        `}
                                        >
                                            ...
                                            <div className="relative aspect-square overflow-hidden bg-gray-100">
                                                <Image
                                                    ...
                                                    fill
                                                    alt={variant?.product_variant_title}
                                                    className={`object-cover duration-500 ${isVariantAvailable ? "group-hover:scale-105" : ""}`}
                                                />
                                    ...
                                    ${isSelected
                                                            ? "bg-black/10"
                                                            : isVariantAvailable ? "group-hover:bg-black/5" : ""
                                                        }
                                `}
                                                />

                                                {/* Selected or Unavailable Badge */}
                                                ...
                                                {/* 🛠️ Variant unavailable label */}
                                                {!isVariantAvailable && (
                                                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                                                        <span className="bg-red-600 text-white text-xs font-bold px-2.5 py-1 rounded-md uppercase tracking-wider shadow">
                                                            Unavailable
                                                        </span>
                                                    </div>
                                                )}
                                            ...
                                            {/* Content */}
                                            <div className="p-4 text-left">
                                                <h4 className={`text-sm font-semibold text-black line-clamp-1 ${!isVariantAvailable ? "line-through text-gray-400" : ""}`}>
                                                    {variant?.product_variant_title ||
                                                        variant?.variant_name ||
                                                        `Variant ${variant?.id}`}
                                                </h4>
                                                ...
                                                <div className="mt-2 flex items-center gap-2">
                                                    <span className={`text-base font-bold text-black ${!isVariantAvailable ? "text-gray-400" : ""}`}>
                                                        ₹{variant?.product_variant_price || productData?.price}
                                                    </span>

                                                    {variant?.product_variant_discount &&
                                                        Number(variant?.product_variant_discount) > Number(variant?.product_variant_price) && (
                                                            <span className="text-xs text-gray-400 line-through">
                                                                ₹{variant?.product_variant_discount}
                                                            </span>
                                                        )}
                    ...
                    {selectedVariant?.sizes?.length > 0 && (
                        <div className="mb-8">
                            ...
                            <div className="flex gap-3 flex-wrap">
                                {selectedVariant?.sizes.map((size: any, index: number) => {
                                    const isAvailable = size?.product_size_status !== false;
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`min-w-[3.5rem] h-14 px-3 rounded-full border text-sm font-medium transition-all flex flex-col items-center justify-center
                            ${!isAvailable
                                        ...
                                        >
                                            <span className={!isAvailable ? "text-[11px]" : ""}>
                                                {isAvailable ? size?.product_size : "Unavailable"}
                                            </span>
                                        </button>
                                    );
                                })}
